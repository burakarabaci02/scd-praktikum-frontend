...
export default function SimulationPanel({
  ...
  function addShipment() {
    setShipments(prev => [...prev, { from: "Hamburg", to: "Rom", quantity: 100 }]);
  }
  ...
  return (
    ...
    >
      ...
      <Accordion title="Transporte">
        {shipments.map((s, idx) => (
          <div key={idx} style={{ marginBottom: "15px", borderBottom: "1px solid #555", paddingBottom: "10px" }}>
            <label>Start</label>
            <select
              value={s.from}
              onChange={e => updateShipment(idx, "from", e.target.value)}
              style={{ width: "100%", padding: "6px", marginBottom: "6px", background: "#111", color: "#eee" }}
            >
              {nodeOptions.map(n => (
            ...
            <input
              ...
              max={2000}
              value={s.quantity}
              onChange={e => updateShipment(idx, "quantity", Number(e.target.value))}
              style={{ width: "100%" }}
            />

            <button
              style={{ marginTop: "8px", background: "#b91c1c", padding: "6px", width: "100%", borderRadius: "4px", border: "none", color: "white", cursor: "pointer" }}
      ...
      <div>
        ...
        <div style={{ fontSize: "13px", marginTop: "4px", opacity: 0.85 }}>
          Hauptroute: {Math.round((shipments.reduce((acc, s) => acc + s.quantity, 0) * mainRouteShare) / 100)} Transporte<br/>
          Alternativroute: {Math.round((shipments.reduce((acc, s) => acc + s.quantity, 0) * (100 - mainRouteShare)) / 100)} Transporte
        </div>
        <input
          type="range"
          min={50}
      ...
      {frozenRouteA && frozenRouteA.length > 0 && (
        <Accordion title="Route Summary">
          <div style={{ fontSize: "13px", marginBottom: "12px" }}>
            <strong>Hauptroute (A):</strong><br />
            {frozenRouteA.join(" → ")}<br />
            Kanten: {frozenRouteA.length - 1}
          </div>

          {frozenRouteB && frozenRouteB.length > 0 && (
            <div style={{ fontSize: "13px" }}>
              <strong>Alternativroute (B):</strong><br />
              {frozenRouteB.join(" → ")}<br />
      ...
      {topBottlenecks && topBottlenecks.length > 0 && (
        <Accordion title="Top 3 Engpässe">
          <table style={{ width: "100%", fontSize: "13px", borderCollapse: "collapse" }}>
            <thead>
              <tr>
                <th style={{ textAlign: "left", paddingBottom: "6px", borderBottom: "1px solid #555" }}>Kante</th>
                <th style={{ textAlign: "right", paddingBottom: "6px", borderBottom: "1px solid #555" }}>Load</th>
              </tr>
            </thead>
            <tbody>
        ...
        <div style={{ fontSize: "13px", display: "flex", flexDirection: "column", gap: "6px" }}>
          <label>
            <input
              ...
              onChange={() => onCentralityModeChange && onCentralityModeChange("none")}
            />
            {" "}Keine
          </label>
          ...
          <label>
            <input
              ...
              onChange={() => onCentralityModeChange && onCentralityModeChange("betweenness")}
            />
          ...
          <label>
            <input
              ...
              onChange={() => onCentralityModeChange && onCentralityModeChange("closeness")}
            />
          ...
          <label>
            <input
              ...
              onChange={() => onCentralityModeChange && onCentralityModeChange("degree")}
            />
            {" "}Degree Centrality
          </label>
        </div>
            ...
            {Object.entries(kpis).map(([key, value]) => (
              ...
              >
                <div style={{ fontSize: "26px", fontWeight: "bold" }}>
                  {value.icon ? value.icon + " " : ""}{value.value}
                </div>
                <div style={{ opacity: 0.7, marginTop: "4px" }}>{value.label}</div>
              </div>
            ))}
          </div>
        </Accordion>
